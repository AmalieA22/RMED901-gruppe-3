...
ANOVAresult <-
  data_wrangled %>% 
  mutate(age = log(age)) %>%
  t.test(age~result,var.equal=TRUE , data = data_wrangled)
ANOVAresult
...
data_wrangled %>% 
  group_by(age, result) %>% 
  wilcox.test(age~result,paired=T, data =data_wrangled) %>%
  broom::tidy()

lmfit <-
  data_wrangled %>% 
  lm(result~age, data = data_wrangled) 
...
Positive_test_depending_Pan_day<-
  data_wrangled %>%
  ggplot2::aes(x=result, y=pan_day)+
  geom_point()+
